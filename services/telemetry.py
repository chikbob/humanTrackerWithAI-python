...
def build_worker_runtime_metrics(*, video_sources: list[dict], worker_statuses: list[dict], events: list[dict], settings: dict) -> dict:
    ...
    online_sources = [status for status in worker_statuses if status.get("is_connected")]
    offline_sources = [status for status in worker_statuses if not status.get("is_connected")]
    now = datetime.now().date()
    today_events = [event for event in events if event.get("timestamp") and datetime.fromtimestamp(event["timestamp"]).date() == now]

    avg_fps = 0.0
    if worker_statuses:
        avg_fps = sum(float(status.get("fps") or 0.0) for status in worker_statuses) / max(1, len(worker_statuses))
    ...
    return {
        ...
        "worker_max_reconnect_count": max((int(status.get("reconnect_count") or 0) for status in worker_statuses), default=0),
